...
int main()
{
    ...
    hFile = mmioOpen( (LPWSTR)fileName,&mi,MMIO_WRITE | MMIO_CREATE);
...
HRESULT RecordAudioStream(MyAudioSink* pMySink)
{
    ...
    hr = WriteWaveHeader((HMMIO)hFile, pwaveformatex, &chunkRiff, &chunkData);
    if (FAILED(hr))
    {
    ...
    while (bDone == FALSE)
    {
        ...
        while (packetLength != 0)
        {
            ...
            hr = pMySink->CopyData(pData, numFramesAvailable, &bDone, pwaveformatex, (HMMIO)hFile);
            EXIT_ON_ERROR(hr);
    ...
    hr = FinishWaveFile((HMMIO)hFile, &chunkData, &chunkRiff);
    if (FAILED(hr))
    {
