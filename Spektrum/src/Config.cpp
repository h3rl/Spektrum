...
namespace config
{
    ...
    extern unsigned int window_fps{ 60 };

    namespace audio
    {
        extern AudioWindowFunction windowfunction{ Blackman };
        extern AudioBarStyle barstyle{ Db };
        extern int bar_count{ (int)(FFT_SIZE_HALF*0.6) };
...
#define CONFIG_SAVE_ENUM(variable, type) file << #variable << " " << static_cast<int>(variable) << std::endl;
    void Save()
    {
        std::fstream file("config.cfg", std::ios::out | std::ios::binary);
...
#define CONFIG_LOAD_ENUM(variable, type) if (key == #variable) { int value; iss >> value; variable = static_cast<type>(value); }
    ...
    void Load()
    {
        std::fstream file("config.cfg", std::ios::in | std::ios::binary);
        ...
        while (std::getline(file, line))
        {
            ...
            if (std::getline(iss, key, ' '))
            {
                ...
                CONFIG_LOAD(window_fps);

                CONFIG_LOAD_ENUM(audio::windowfunction, AudioWindowFunction);
                CONFIG_LOAD_ENUM(audio::barstyle, AudioBarStyle);
        ...
        audio::max_db = clamp(audio::max_db, audio::min_db, 0.0f);
